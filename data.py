...
class GetMRI(Dataset):
    ...
    def __getitem__(self, index):
        ...
        siat_temp = np.zeros((256, 256, 2))
        siat_temp[:,:,0] = siat[:,:,0]
        siat_temp[:,:,1] = siat[:,:,1]
        siat_data = np.array(siat_temp,dtype=np.float32)
        ...
        if FLAGS.datasource == 'default':
            im_corrupt = siat_data + 0.3 * np.random.randn(image_size, image_size, 2)
        elif FLAGS.datasource == 'random':
            im_corrupt = np.random.uniform(
                -1.0, 1.0, (image_size, image_size, 2))
...
class GetCImageNetData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_data = im_data + np.random.uniform( self.rescale *(-1) / 256. , self.rescale * 1 / 256., im.shape)
        np.random.seed((index + int(time.time() * 1e7)) % 2**32)
        ...
        if FLAGS.datasource == 'default':
            ...
            im_corrupt = np.random.uniform((-1)*self.rescale, self.rescale, (image_size, image_size, 3))
