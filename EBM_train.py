...
from data import Imagenet, Cifar10, DSprites, Mnist, TFImagenetLoader,GetMRI,GetCImageNetData
...
# Dataset Options
flags.DEFINE_string('datasource', 'default',
...
flags.DEFINE_integer('test_interval', 250,'evaluate outputs every so many batches')
...
flags.DEFINE_string('objective', 'logsumexp', 'use either contrastive divergence objective(least stable),'
...
def compress_x_mod(x_mod):
    x_mod = (x_mod + 1) / 2 # (-1,1)--->(0,1)
    x_mod = (255 * np.clip(x_mod, 0, 1))# (0,1)--->(0,255)
    return x_mod


def decompress_x_mod(x_mod):
    x_mod = x_mod / 255  #(0,255)--->(0,1)
    x_mod = 2*x_mod - 1 #(0,1)--->(-1,1)
    x_mod = x_mod * 255 / 256 * FLAGS.rescale + np.random.uniform(-1 / 256 * FLAGS.rescale, 1 / 256 * FLAGS.rescale, x_mod.shape)
    return x_mod
...
def rescale_im(image):
    ...
    if FLAGS.dataset == 'mnist' or FLAGS.dataset == 'dsprites':
        return (np.clip((FLAGS.rescale - image) * 256 / FLAGS.rescale, 0, 255))
    else:
        return (np.clip(image * 256 / FLAGS.rescale, 0, 255))
    ...
    for epoch in range(FLAGS.epoch_num):
        ...
        for data_corrupt, data, label in dataloader: 
            ...
            if itr % FLAGS.test_interval == 0 and hvd.rank() == 0 and FLAGS.dataset != '2d': 
                ...
                for i, (im, t_im, actual_im_i) in enumerate(zip(orig_im[:20], try_im[:20], actual_im)):
                ...
                if FLAGS.dataset == 'cifar10' or FLAGS.dataset == 'imagenet' or FLAGS.dataset == 'imagenetfull' or FLAGS.dataset == 'fastMRI' or  FLAGS.dataset == 'CImageNet':
                    n = 16
                    ...
                    if len(replay_buffer) > n: 
                        ...
                        data_corrupt = np.random.uniform( (-1)*FLAGS.rescale, FLAGS.rescale, (n, 256, 256, 2)) 
...
def main():
    ...
    if FLAGS.dataset == 'fastMRI':
        dataset = GetMRI(root = '../siat_256size')
        test_dataset = GetMRI(root = '../siat_256size')
        channel_num = 2
        
        X_NOISE = tf.placeholder(shape=(None, 256, 256, 2), dtype=tf.float32)
        X = tf.placeholder(shape=(None, 256, 256, 2), dtype=tf.float32)
    ...
    elif FLAGS.dataset == 'CImageNet':

        dataset = GetCImageNetData(root = '../CImageNet300_train_3h_png')
        test_dataset = GetCImageNetData(root = '../CImageNet300_train_3h_png')
        channel_num = 3
    ...
    for j in range(FLAGS.num_gpus):
        ...
        if FLAGS.model_cclass: 
            ...
            energy_pos = [model.forward(X_SPLIT[j],weights[0],label=LABEL_POS_SPLIT[j],stop_at_grad=False)] 
            energy_pos = tf.concat(energy_pos, axis=0)
        ...
        energy_negs.extend([model.forward(tf.stop_gradient(x_mod), weights[0], label=LABEL_SPLIT[j], stop_at_grad=False, reuse=True)])
        ...
        def langevin_step(counter, x_mod):
            ...
            x_mod = x_last
            x_mod = tf.clip_by_value(x_mod, (-1)*FLAGS.rescale, FLAGS.rescale)
        ...
        energy_eval = model.forward(x_mod, weights[0], label=LABEL_SPLIT[j],stop_at_grad=False, reuse=True)
        x_grad = tf.gradients(FLAGS.temperature * energy_eval, [x_mod])[0]
        x_grads.append(x_grad)

        energy_negs.append(model.forward(
        ...
        loss_energy = model.forward(x_mod,weights[0],reuse=True,label=LABEL,stop_grad=True)
